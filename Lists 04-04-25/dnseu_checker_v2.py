...
dns_server = "193.110.81.1"  # DNS0.eu Kids
max_queries_per_day = 290000
queries_per_second = max_queries_per_day / 86400  # ≈ 3.356
...
async def check_domain(domain, blocked_file, allowed_file, blocked_set, allowed_set, index, total):
    async with semaphore:
        try:
            ...
            # If there's an error or no answer, treat as blocked
            blocked_set.add(domain)
            await save_result(blocked_file, domain)
...
async def main(domain_file):
    ...
    tasks = [
        check_domain(d, blocked_file, allowed_file, blocked, allowed, idx + 1, total)
        for idx, d in enumerate(to_check)
    ]
